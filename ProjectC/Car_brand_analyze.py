# -*- coding: utf-8 -*-
"""
Created on Sun Aug  2 15:11:22 2020
...
data = pd.read_csv('E:/python/svw数据分析实战/exam/ProjectC/CarPrice_Assignment.csv')
print(data.shape)
#print(data.head())
...
def training_data_norm(data):
    ...
    for item in symbolic_index:
        symbols = data[item].unique() #每一列都取唯一值
...
def vw_competetor(result): # 提取品牌为大众的记录
    result_vw = pd.concat([result[result['CarBrand'] == 'vw'],
                           result[result['CarBrand'] == 'volkswagen'],
                           result[result['CarBrand']=='vokswagen']])
    competetor = []
    # 获取和volkswagen品牌相同聚类结果的品牌
    for i in result_vw['聚类结果'].unique():
        competetor += list(result[result['聚类结果'] == i]['CarBrand'].unique())  # 品牌去重
...
def main():
    elbow_method(data)
    result = K_means(data, n_clusters=7)
    #result.to_csv('111.csv', encoding='gbk')
